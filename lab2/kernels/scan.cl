...
// this code is borrowed from https://github.com/nchong/scan

/*
 * Inplace upsweep (reduce) on a local array [x] of length [m].
 * NB: [m] must be a power of two.
 */
inline void upsweep_pow2(__local element_t *x, int m) {
    ...
    for (int d = 0; d < depth; d++) {
        ...
        // while going up, for depth d partial results are evaluated in every 2^(d+1) position
        // (see pics on http://http.developer.nvidia.com/GPUGems3/gpugems3_ch39.html)
        if ((lid & mask) == mask) {
            int offset = (0x1 << d);
            int ai = bi - offset;
            x[bi] += x[ai];
 ...
 * NB: [m] must be a power of two.
 */
inline void sweepdown_pow2(__local element_t *x, int m) {
    int lid = get_local_id(0);
    int bi = (lid * 2) + 1;
 ...
 * NB: m must be a power of two.
 */
inline void scan_pow2(__local element_t *x, int m) {
    int lid = get_local_id(0);
    int lane1 = (lid * 2) + 1;
    upsweep_pow2(x, m);
    if (lane1 == (m - 1)) {
...
__kernel void scan_pad_to_pow2(__global element_t *data, __local element_t *x, int n) {
...
__kernel void scan_subarrays(
        __global element_t *data, //length [n]
        __local element_t *x,    //length [m]
        __global element_t *part, //length [m]
        int n
) {
    // workgroup size
